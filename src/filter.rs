...
impl fmt::Display for InnerFilter {
...
fn parse_spec(spec: &str) -> (Vec<Directive>, Option<InnerFilter>) {
    ...
    let filter = filter.map_or(None, |filter| match InnerFilter::new(filter) {
        Ok(re) => Some(re),
        Err(e) => {
